...
function NavSidebar({ activeSection, setActiveSection }) {
  ...
  return (
    <>
      ...
      <nav 
        className={`fixed top-0 left-0 w-64 h-screen bg-lime-50  p-6 shadow-xl flex-col z-50 transition-transform duration-300 ease-in-out
      ...
      >
        ...
        <button 
          onClick={() => setIsSidebarOpen(false)}
          className="absolute top-4 right-4 p-2 text-gray-500 hover:text-gray-800 sm:hidden"
        >
        ...
        <div className="flex flex-col justify-between h-full">
          <div>
            <div className="text-3xl font-bold text-teal-400 mb-10 text-center inline-flex items-baseline">
              <SproutLogo className="h-8 w-10 mr-2" />
              <span>투싹증권</span>
            </div>
            <ul>
              {navItems.map((item) => {
